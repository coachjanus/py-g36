...
def _create_databse(db_path):
    canfig_parser = configparser.ConfigParser()
    canfig_parser['General'] = {"database": db_path}
    
    try:
        with CONFIG_FILE_PATH.open('w') as file:
            canfig_parser.write(file)
    except OSError:
...
def init_app(db_path):
    ...
    if config_code != SUCCESS:
        return config_code
    database_code = _create_databse(db_path)
